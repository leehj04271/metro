...
if ($("#wrap").is(".trainInfo")) {
    ...
    if (history.state) {
        ...
        const cur_statn_idx = line2.findIndex(
            (station) => station.statn_nm === state
        );
        ...
        $(".station-prev").text(
            line2[cur_statn_idx - 1]
                ? slice(line2[cur_statn_idx - 1].statn_nm)
                : "아현"
        );
...
function insertData(arrivalDataArr) {
    ...
    [
        arrivalDataArr.filter((i) => i.updnLine === "내선"),
        arrivalDataArr.filter((i) => i.updnLine !== "내선"),
    ].forEach((line) => {
        console.log(line);
		...
		${
            line[0].updnLine === "내선"
                ...
                : `
					<div  class='train-msg'> ${line[0].arvlMsg3}  ·  ${line[0].arvlMsg2} ${
                      line[0].arvlMsg2.slice(-2) !== " 후" ? "" : "도착"
                  } 
